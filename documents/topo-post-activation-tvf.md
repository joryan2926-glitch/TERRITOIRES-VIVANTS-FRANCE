...
| Resend | OK | Après remplacement de la clé, notification interne et accusé de réception sont envoyés. |
...
| Vérifier les logs Resend détaillés | Il faut l’accès au compte Resend. | Resend > Transactionnel > Logs > chercher les objets TEST RESEND / TEST RECEPTION. |
...
3. Dans Resend, confirmer le statut `delivered` des derniers tests.
4. Dans Vercel, vérifier :
   - `EMAIL_PROVIDER=resend`
   - `RESEND_API_KEY` actif
