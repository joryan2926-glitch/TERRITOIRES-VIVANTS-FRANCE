...
function taskFilePanel(item = {}) {
  const linkedCase = taskIsLinkedToCase(item);
  const assistant = item.assistant || {};
  const dueLabel = item.due_at ? formatDate(item.due_at) : "Echeance a definir";
  const expected = item.description || item.summary || item.ai_summary || "Resultat attendu a preciser avant execution.";
  return `<section class="work-file-panel" aria-label="Fiche de travail"><div class="admin-panel-head"><div><p class="section-kicker">Fiche de travail</p><h3>${escapeHtml(titleFor(item))}</h3><p>Vue courte pour comprendre l'action, son rattachement, son responsable et la prochaine decision sans changer de module.</p></div><strong>${escapeHtml(label(item.status))}</strong></div><div class="work-file-grid"><article><span>Responsable</span><strong>${escapeHtml(item.assigned_to || "A affecter")}</strong><small>${escapeHtml(item.pole || "Pole non renseigne")}</small></article><article><span>Echeance</span><strong>${escapeHtml(dueLabel)}</strong><small>${taskIsOverdue(item) ? "Retard a traiter" : "Suivi courant"}</small></article><article><span>Rattachement</span><strong>${escapeHtml(linkedCase ? "Dossier" : label(item.related_object_type || "none"))}</strong><small>${escapeHtml(item.related_object_id || "Aucun objet lie")}</small></article><article><span>Priorite</span><strong>${escapeHtml(label(item.priority))}</strong><small>${escapeHtml(assistant.summary || "Action a suivre")}</small></article></div><div class="work-file-brief"><article><h4>Resultat attendu</h4><p>${escapeHtml(expected)}</p></article><article><h4>Suite conseillee</h4><p>${escapeHtml((assistant.next_actions || ["Mettre a jour le statut", "Tracer la suite"])[0])}</p></article></div></section>`;
}

function taskActionsPanel(item = {}) {
  const linkedCase = taskIsLinkedToCase(item);
  return `<section class="work-action-panel"><div class="admin-panel-head"><div><p class="section-kicker">Actions de suivi</p><h3>Piloter la tache sans dispersion</h3><p>Les actions rapides couvrent l'avancement, le report, la priorisation et le retour vers les modules rattaches.</p></div></div><div class="work-action-groups"><article><h4>1. Avancement</h4><button class="btn secondary" type="button" data-task-quick="doing">Demarrer</button><button class="btn secondary" type="button" data-task-quick="waiting">Mettre en attente</button><button class="btn primary" type="button" data-task-quick="done">Terminer</button></article><article><h4>2. Echeance</h4><button class="btn secondary" type="button" data-task-replan="48h">Reporter 48h</button><button class="btn secondary" type="button" data-task-replan="7d">Reporter 7 jours</button></article><article><h4>3. Priorite</h4><button class="btn secondary" type="button" data-task-priority="P1">Passer P1</button><button class="btn secondary" type="button" data-task-priority="P2">Passer P2</button></article><article><h4>4. Modules lies</h4><a class="btn secondary" href="admin-dossiers">Dossiers</a><a class="btn secondary" href="admin-documents">Documents</a><a class="btn secondary" href="admin-crm">CRM</a>${linkedCase ? `<button class="btn secondary" type="button" data-copy-related-id="${escapeHtml(item.related_object_id)}">Copier ID dossier</button>` : ""}</article></div></section>`;
}
function renderDetail() {
  ...
  const projectPanels = view === "projects" ? `${projectPortfolioPanel(item)}${projectControlPanel(item)}` : view === "tasks" ? `${taskFilePanel(item)}${taskControlPanel(item)}${taskActionsPanel(item)}` : "";
...
async function replanTask(period = "48h") {
  const item = selectedItem();
  if (!item || view !== "tasks") return;
  const due = new Date();
  if (period === "7d") due.setDate(due.getDate() + 7);
  else due.setDate(due.getDate() + 2);
  due.setHours(17, 0, 0, 0);
  ...
  await loadAll();
  notify(period === "7d" ? "Tache reportee de 7 jours." : "Tache replanifiee a 48h.", "success");
}

async function updateTaskPriority(priority) {
  const item = selectedItem();
  if (!item || view !== "tasks") return;
  await api("/api/admin-work", { method: "PATCH", body: JSON.stringify({ type: "task", id: item.id, priority }) });
  await loadAll();
  notify(`Priorite ${priority} appliquee.`, "success");
}
...
async function copyRelatedId(id) {
  ...
  try {
    ...
    notify("ID rattache copie.", "success");
  } catch {
    notify("Copie automatique impossible. Copiez l'ID affiche dans la fiche.", "warning");
  }
}
