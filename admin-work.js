...
function taskActionsPanel(item = {}) {
  const linkedCase = taskIsLinkedToCase(item);
  const relatedLink = linkedCase ? `admin-dossiers?q=${encodeURIComponent(item.related_object_id)}` : "admin-dossiers";
  return `<section class="work-action-panel"><div class="admin-panel-head"><div><p class="section-kicker">Action directe</p><h3>Traiter la tache</h3><p>Un seul bloc pour avancer, reporter ou ouvrir le module utile.</p></div></div><div class="work-action-row"><button class="btn secondary" type="button" data-task-quick="doing">Demarrer</button><button class="btn secondary" type="button" data-task-quick="waiting">Mettre en attente</button><button class="btn primary" type="button" data-task-quick="done">Terminer</button><button class="btn secondary" type="button" data-task-replan="7d">Reporter 7 jours</button><button class="btn secondary" type="button" data-task-priority="P1">Priorite P1</button></div><div class="work-action-links"><a class="text-link" href="${escapeHtml(relatedLink)}">Dossier lie</a><a class="text-link" href="admin-documents">Documents</a><a class="text-link" href="admin-crm">CRM</a>${linkedCase ? `<button class="text-link button-link" type="button" data-copy-related-id="${escapeHtml(item.related_object_id)}">Copier ID</button>` : ""}</div></section>`;
}
