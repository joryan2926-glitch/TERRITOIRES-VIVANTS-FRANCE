...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      if (type === "decision") return sendJson(res, 201, { ok: true, decision: await createRecord("decisions", decisionPayload(input), req, type) });
      if (type === "committee") return sendJson(res, 201, { ok: true, committee: await createRecord("committees", committeePayload(input), req, type) });
      if (type === "item") return sendJson(res, 201, { ok: true, item: await createRecord("committee_items", itemPayload(input), req, type) });
      if (type === "vote") return sendJson(res, 201, { ok: true, vote: await createRecord("decision_votes", votePayload(input), req, type) });
      if (type === "delegation") return sendJson(res, 201, { ok: true, delegation: await createRecord("delegations", delegationPayload(input), req, type) });
      if (type === "action") return sendJson(res, 201, { ok: true, action: await createRecord("decision_actions", actionPayload(input), req, type) });
    ...
    if (req.method === "PATCH") {
      ...
      if (type === "decision") return sendJson(res, 200, { ok: true, decision: await updateRecord("decisions", id, decisionPayload(input, true), req, type) });
      if (type === "committee") return sendJson(res, 200, { ok: true, committee: await updateRecord("committees", id, committeePayload(input, true), req, type) });
      if (type === "item") return sendJson(res, 200, { ok: true, item: await updateRecord("committee_items", id, itemPayload(input, true), req, type) });
      if (type === "delegation") return sendJson(res, 200, { ok: true, delegation: await updateRecord("delegations", id, delegationPayload(input, true), req, type) });
      if (type === "action") return sendJson(res, 200, { ok: true, action: await updateRecord("decision_actions", id, actionPayload(input, true), req, type) });
    }
    sendJson(res, 405, { ok: false, error: "Methode non autorisee." });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    sendJson(res, statusCode, { ok: false, error: statusCode >= 500 ? "Gouvernance temporairement non chargee : vérifiez la configuration Supabase du module." : error.message, code: error.code || undefined, details: process.env.NODE_ENV === "development" ? error.details || error.message : undefined });
  }
};
