...
function crmAssistant(entity, type) {
  ...
  return {
    summary: entity.ai_summary || `Contact ${entity.contact_type || inferContactType(entity)} - fiche relationnelle a qualifier.`,
    missing_fields: missing,
    next_action: nextAction,
    next_action_due_at: entity.next_action_due_at || addDaysIso(14),
...
async function createRow(table, payload, enrich, req, type) {
  const params = new URLSearchParams();
  params.set("select", "*");
  ...
  const record = enrich ? enrich(rows?.[0] || null) : rows?.[0] || null;
  await writeAdminAudit({ supabaseRequest, req, moduleKey: "crm", action: `create_${type || table}`, objectType: type || table, objectId: record?.id, summary: `Creation ${type || table} dans le CRM TVF OS.`, metadata: { table, relation_status: record?.relation_status || null, contact_type: record?.contact_type || null, consent_status: record?.consent_status || null } });
  return record;
}

async function updateRow(table, id, payload, enrich, req, type) {
  ...
  const record = enrich ? enrich(rows?.[0] || null) : rows?.[0] || null;
  await writeAdminAudit({ supabaseRequest, req, moduleKey: "crm", action: `update_${type || table}`, objectType: type || table, objectId: record?.id || cleanId, summary: `Mise a jour ${type || table} dans le CRM TVF OS.`, metadata: { table, changed_fields: Object.keys(payload || {}), relation_status: record?.relation_status || null, consent_status: record?.consent_status || null } });
  return record;
}
...
async function dashboardCrm() {
  ...
  return {
    ...
    contacts_to_qualify: activeContacts.filter((item) => !item.email && !item.phone && !item.mobile).length,
    ...
    collectivites: activeOrgs.filter((item) => item.organization_type === "collectivite").length,
    entreprises: activeOrgs.filter((item) => item.organization_type === "entreprise").length + activeContacts.filter((item) => item.contact_type === "entreprise_contact").length,
    financeurs: activeOrgs.filter((item) => item.organization_type === "financeur").length + activeContacts.filter((item) => item.contact_type === "financeur").length,
    proprietaires: activeOrgs.filter((item) => item.organization_type === "proprietaire_personne_morale").length + activeContacts.filter((item) => item.contact_type === "proprietaire").length,
  };
}
...
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      if (type === "contact") return sendJson(res, 201, { ok: true, contact: await createRow("crm_contacts", contactPayload(data), enrichContact, req, type) });
      if (type === "organization") return sendJson(res, 201, { ok: true, organization: await createRow("organizations", organizationPayload(data), enrichOrganization, req, type) });
      if (type === "history") return sendJson(res, 201, { ok: true, item: await createRow("relationship_history", historyPayload(data), null, req, type) });
      if (type === "link") return sendJson(res, 201, { ok: true, link: await createRow("organization_contacts", linkPayload(data), null, req, type) });
    }

    if (req.method === "PATCH") {
      if (type === "contact") return sendJson(res, 200, { ok: true, contact: await updateRow("crm_contacts", data.id, contactPayload(data, true), enrichContact, req, type) });
      if (type === "organization") return sendJson(res, 200, { ok: true, organization: await updateRow("organizations", data.id, organizationPayload(data, true), enrichOrganization, req, type) });
      if (type === "duplicate") {
        const status = clean(data.status || "", 80);
        validateEnum(DUPLICATE_STATUS, status, "Statut de doublon invalide.");
        return sendJson(res, 200, { ok: true, duplicate: await updateRow("crm_duplicate_suggestions", data.id, { status, reviewed_by: clean(data.reviewed_by || "TVF OS", 180), reviewed_at: new Date().toISOString() }, null, req, type) });
      }
    }
    ...
    const setupError = statusCode >= 500 ? setupErrorMessage(error, "CRM", "supabase/tvf-os-crm.sql") : null;
    sendJson(res, setupError ? 503 : statusCode, {
      ok: false,
      error: setupError ? setupError.message : statusCode >= 500 ? "CRM temporairement non charge : vérifiez la configuration Supabase du module." : error.message,
      code: setupError?.code || error.code || undefined,
