...
function resolveInternalKitPath(relativePath) {
  ...
  const allowed = INTERNAL_KIT_FILES.some(([, , , itemPath]) => itemPath === cleanPath);
  if (!allowed) throw Object.assign(new Error("Document interne non autorise."), { statusCode: 403 });
...
function internalKitCatalog() {
  ...
  for (const [title, group, description, relativePath] of INTERNAL_KIT_FILES) {
    let size = 0;
