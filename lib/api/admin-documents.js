...
const INTERNAL_KIT_ROOT = path.join(process.cwd(), "documents", "kit-formulaires-conventions-tvf");
const INTERNAL_KIT_ARCHIVE = path.join(process.cwd(), "documents", "TVF-kit-formulaires-conventions-prets-a-utiliser.zip");
const INTERNAL_KIT_FILES = [
  ["Kit complet", "Archive ZIP", "Tous les formulaires, conventions, courriers et listes de pieces.", "documents/TVF-kit-formulaires-conventions-prets-a-utiliser.zip"],
  ["Index", "00 - Index", "Sommaire du kit et logique de classement interne.", "documents/kit-formulaires-conventions-tvf/00-index/tvf-index-kit-formulaires-conventions.docx"],
  ["Contact general", "01 - Formulaires", "Qualifier une demande entrante et l'orienter vers le bon parcours.", "documents/kit-formulaires-conventions-tvf/01-contact-general/tvf-f-01-formulaire-de-contact-general.docx"],
  ["Collectivite / territoire", "01 - Formulaires", "Preparer une demande de rendez-vous, diagnostic ou cooperation.", "documents/kit-formulaires-conventions-tvf/02-collectivite-territoire/tvf-f-02-demande-collectivite---territoire-partenaire.docx"],
  ["Proprietaire / bien vacant", "01 - Formulaires", "Etudier un logement, commerce, local, terrain ou friche.", "documents/kit-formulaires-conventions-tvf/03-proprietaire-bien-vacant/tvf-f-03-proposition-de-bien-vacant-ou-inutilise.docx"],
  ["Materiaux de reemploi", "01 - Formulaires", "Qualifier des materiaux, mobiliers ou equipements disponibles.", "documents/kit-formulaires-conventions-tvf/04-materiaux-reemploi/tvf-f-04-proposition-de-materiaux-reemployables.docx"],
  ["Entreprise partenaire", "01 - Formulaires", "Recueillir une proposition de materiaux, local, transport, competences ou soutien.", "documents/kit-formulaires-conventions-tvf/05-entreprise-partenariat/tvf-f-05-demande-entreprise---partenariat-territorial.docx"],
  ["Local de stockage", "01 - Formulaires", "Etudier un local potentiel pour stockage, tri et logistique.", "documents/kit-formulaires-conventions-tvf/06-local-stockage/tvf-f-06-mise-a-disposition-potentielle-dun-local-de-stockage.docx"],
  ["Transport / logistique", "01 - Formulaires", "Recenser vehicule, chauffeur, manutention ou livraison possible.", "documents/kit-formulaires-conventions-tvf/07-transport-logistique/tvf-f-07-mise-a-disposition-transport-et-logistique.docx"],
  ["Benevole / citoyen", "01 - Formulaires", "Identifier competences, disponibilites et missions possibles.", "documents/kit-formulaires-conventions-tvf/08-benevole-engagement/tvf-f-08-candidature-benevole-ou-citoyenne.docx"],
  ["Association / insertion", "01 - Formulaires", "Construire une action encadree, sociale ou participative.", "documents/kit-formulaires-conventions-tvf/09-association-insertion-chantier/tvf-f-09-cooperation-association-insertion-ou-chantier-participatif.docx"],
  ["Financeur / mecene", "01 - Formulaires", "Structurer un soutien financier, materiel ou en competences.", "documents/kit-formulaires-conventions-tvf/10-financeur-mecene/tvf-f-10-demande-financeur-mecene-ou-investisseur-solidaire.docx"],
  ["Signalement citoyen", "01 - Formulaires", "Signaler un lieu, une friche ou une ressource a verifier.", "documents/kit-formulaires-conventions-tvf/11-signalement-citoyen/tvf-f-11-signalement-citoyen-dun-lieu-ou-dune-ressource.docx"],
  ["Presse / institution", "01 - Formulaires", "Traiter une demande d'information, d'interview ou de communication.", "documents/kit-formulaires-conventions-tvf/12-presse-institutionnel/tvf-f-12-demande-presse-institution-ou-communication.docx"],
  ["Convention cooperation territoriale", "02 - Conventions", "Cadrer une cooperation avec une collectivite ou un EPCI.", "documents/kit-formulaires-conventions-tvf/13-conventions-types/tvf-conv-01-convention-type-de-cooperation-territoriale.docx"],
  ["Convention mise a disposition d'un bien", "02 - Conventions", "Encadrer etude, visite, usage temporaire ou restitution.", "documents/kit-formulaires-conventions-tvf/13-conventions-types/tvf-conv-02-convention-type-de-mise-a-disposition-dun-bien.docx"],
  ["Convention valorisation de materiaux", "02 - Conventions", "Tracer collecte, tri, securite, affectation et responsabilites.", "documents/kit-formulaires-conventions-tvf/13-conventions-types/tvf-conv-03-convention-type-de-valorisation-de-materiaux.docx"],
  ["Convention partenariat entreprise", "02 - Conventions", "Formaliser contribution RSE, visibilite, reporting et limites.", "documents/kit-formulaires-conventions-tvf/13-conventions-types/tvf-conv-04-convention-type-de-partenariat-entreprise.docx"],
  ["Convention soutien financier ou mecenat", "02 - Conventions", "Cadrer affectation, versement, justificatifs et communication.", "documents/kit-formulaires-conventions-tvf/13-conventions-types/tvf-conv-05-convention-type-de-soutien-financier-ou-mecanat.docx"],
  ["Charte benevole et chantier participatif", "02 - Conventions", "Fixer securite, mission, emargement, confidentialite et image.", "documents/kit-formulaires-conventions-tvf/13-conventions-types/tvf-conv-06-charte-type-benevole-et-chantier-participatif.docx"],
  ["Liste des pieces par demande", "03 - Pieces a fournir", "Verifier les pieces minimales selon le type de dossier.", "documents/kit-formulaires-conventions-tvf/14-listes-pieces/tvf-lp-01-liste-pieces-par-demande.docx"],
  ["Kit courriers prets a envoyer", "04 - Courriers", "Modeles de courriers ranges par interlocuteur et besoin.", "documents/kit-formulaires-conventions-tvf/15-courriers-prets-a-envoyer/tvf-cour-01-kit-courriers-demandes.docx"],
];

function kitPathToRelative(filePath) {
  return path.relative(process.cwd(), filePath).replace(/\\/g, "/");
}
function walkInternalKitFiles(dir) {
  if (!fs.existsSync(dir)) return [];
  return fs.readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
    const full = path.join(dir, entry.name);
    return entry.isDirectory() ? walkInternalKitFiles(full) : [full];
  });
}
function kitGroupFromPath(relativePath) {
  const folder = relativePath.split("/")[2] || "";
  if (folder === "00-index") return "00 - Index";
  if (folder === "13-conventions-types") return "02 - Conventions";
  if (folder === "14-listes-pieces") return "03 - Pieces a fournir";
  if (folder === "15-courriers-prets-a-envoyer") return "04 - Courriers";
  if (folder === "16-instruction-dossier") return "05 - Instruction dossier";
  if (/^\d{2}-/.test(folder)) return "01 - Formulaires";
  return "06 - Autres documents";
}
function titleFromKitFilename(relativePath) {
  const base = path.basename(relativePath, ".docx")
    .replace(/^tvf-(f|conv|lp|inst|cour)-\d+-/i, "")
    .replace(/^tvf-index-/i, "index-")
    .replace(/---/g, "-")
    .replace(/-/g, " ")
    .replace(/\s+/g, " ")
    .trim();
  return base ? base.charAt(0).toUpperCase() + base.slice(1) : path.basename(relativePath);
}
function descriptionFromKitPath(relativePath) {
  const file = path.basename(relativePath).toLowerCase();
  if (file.includes("index")) return "Index de classement et logique d'utilisation du kit interne.";
  if (file.startsWith("tvf-f-")) return "Formulaire pret a remplir pour ouvrir ou qualifier une demande.";
  if (file.startsWith("tvf-lp-")) return "Liste de pieces a fournir avec controles, observations et decision TVF.";
  if (file.startsWith("tvf-inst-")) return "Fiche interne TVF OS pour ouvrir, instruire, visiter, decider ou suivre un dossier.";
  if (file.startsWith("tvf-conv-")) return "Modele de convention, autorisation, charte ou proces-verbal a adapter avant signature.";
  if (file.startsWith("tvf-cour-")) return "Kit de courriers prets a adapter pour demandes, relances et partenariats.";
  return "Document interne TVF OS pret a completer.";
}
function internalKitEntries() {
  const metadata = new Map(INTERNAL_KIT_FILES.map((item) => [item[3], item]));
  const entries = [];
  const archiveRelative = "documents/TVF-kit-formulaires-conventions-prets-a-utiliser.zip";
  entries.push(metadata.get(archiveRelative) || ["Kit complet", "Archive ZIP", "Tous les documents internes TVF OS.", archiveRelative]);
  const files = walkInternalKitFiles(INTERNAL_KIT_ROOT)
    .filter((filePath) => filePath.toLowerCase().endsWith(".docx"))
    .map(kitPathToRelative)
    .sort((a, b) => a.localeCompare(b));
  for (const relativePath of files) {
    entries.push(metadata.get(relativePath) || [titleFromKitFilename(relativePath), kitGroupFromPath(relativePath), descriptionFromKitPath(relativePath), relativePath]);
  }
  return entries;
}
...
function setupErrorMessage(error, moduleLabel, migrationPath) {
  const detailText = String(error?.details || error?.message || "").toLowerCase();
  const code = String(error?.code || "").toLowerCase();
  const isSupabaseSetupIssue =
    code === "42p01" ||
    detailText.includes("does not exist") ||
    detailText.includes("relation") ||
    detailText.includes("schema cache") ||
    detailText.includes("could not find") ||
    detailText.includes("storage.buckets") ||
    detailText.includes("tvf-documents") ||
    detailText.includes("supabase admin non configure");

  if (isSupabaseSetupIssue) {
    const setupError = new Error(`${moduleLabel} a configurer : executez ${migrationPath}, puis le fichier de verification associe dans Supabase.`);
    setupError.code = "MODULE_SETUP_REQUIRED";
    return setupError;
  }
...
function documentTypeFromTemplateType(templateType) {
  return {
    courrier: "courrier",
    email: "email",
...
async function listDocuments(req) {
  ...
  if (/^[0-9a-f-]{32,36}$/i.test(relatedId)) params.set("related_object_id", `eq.${relatedId}`);
...
async function dashboardDocuments() {
  const docs = await supabaseRequest("documents?select=id,status,document_type,confidentiality_level,sensitive_detected,expires_at,created_at,related_object_type,related_object_id&limit=1000");
  ...
  return {
    ...
    linked_cases: docs.filter((d) => d.related_object_type === "case" && d.related_object_id).length,
    non_rattaches: docs.filter((d) => !d.related_object_id || !d.related_object_type || d.related_object_type === "none").length,
...
async function createDocument(input, req) {
  ...
  await writeAdminAudit({ supabaseRequest, req, moduleKey: "documents", action: "create_document", objectType: "document", objectId: document?.id, summary: `Creation du document ${document?.document_number || document?.title || "TVF"}.`, metadata: { document_type: document?.document_type || payload.document_type, status: document?.status || payload.status, related_object_type: document?.related_object_type || payload.related_object_type, related_object_id: document?.related_object_id || payload.related_object_id || null } });
  return enrichDocument(document);
}
async function updateDocument(input, req) {
  ...
  await writeAdminAudit({ supabaseRequest, req, moduleKey: "documents", action: "update_document", objectType: "document", objectId: rows?.[0]?.id || id, summary: `Mise a jour du document ${rows?.[0]?.document_number || rows?.[0]?.title || id}.`, metadata: { changed_fields: Object.keys(payload || {}), status: rows?.[0]?.status || payload.status || null } });
  return enrichDocument(rows?.[0]);
}
async function createTemplate(input, req) {
  const createdFile = await createFileFromPayload(input);
  ...
  const template = enrichTemplate(rows?.[0]);
  await writeAdminAudit({ supabaseRequest, req, moduleKey: "documents", action: "create_template", objectType: "template", objectId: template?.id, summary: `Creation du modele ${template?.title || "TVF"}.`, metadata: { template_type: template?.template_type || payload.template_type, status: template?.status || payload.status } });
  return template;
}
async function updateTemplate(input, req) {
  const id = clean(input.id || "", 80);
  ...
  const template = enrichTemplate(rows?.[0]);
  await writeAdminAudit({ supabaseRequest, req, moduleKey: "documents", action: "update_template", objectType: "template", objectId: template?.id || id, summary: `Mise a jour du modele ${template?.title || id}.`, metadata: { changed_fields: Object.keys(payload || {}), status: template?.status || payload.status || null } });
  return template;
}
async function generateDocument(input, req) {
  ...
  const document = await createDocument({
    ...
    document_type: documentTypeFromTemplateType(template.template_type),
    ...
    ai_summary: missing.length ? `Champs manquants : ${missing.join(", ")}.` : `Brouillon genere depuis le modele ${template.title}. Validation humaine requise avant envoi ou signature.`,
    classification_notes: clean(JSON.stringify({ template_type: template.template_type, field_values: fieldValues }), 1600),
  }, req);
  ...
  await writeAdminAudit({ supabaseRequest, req, moduleKey: "documents", action: "generate_document", objectType: "generated_document", objectId: generatedRows?.[0]?.id, summary: `Generation documentaire depuis le modele ${template.title}.`, metadata: { template_id: templateId, document_id: document?.id, generation_status: generationStatus, missing_fields: missing } });
  return { document, generated: generatedRows?.[0] || null };
}
function resolveInternalKitPath(relativePath) {
  const cleanPath = clean(relativePath || "", 500).replace(/\\/g, "/");
  const isArchive = cleanPath === "documents/TVF-kit-formulaires-conventions-prets-a-utiliser.zip";
  const isKitDocx = cleanPath.startsWith("documents/kit-formulaires-conventions-tvf/") && cleanPath.toLowerCase().endsWith(".docx");
  if (!isArchive && !isKitDocx) throw Object.assign(new Error("Document interne non autorise."), { statusCode: 403 });
  const absolutePath = path.resolve(process.cwd(), cleanPath);
  const kitRoot = path.resolve(INTERNAL_KIT_ROOT);
  const archivePath = path.resolve(INTERNAL_KIT_ARCHIVE);
  if (absolutePath !== archivePath && !absolutePath.startsWith(`${kitRoot}${path.sep}`)) {
    throw Object.assign(new Error("Chemin document invalide."), { statusCode: 403 });
  }
  if (!fs.existsSync(absolutePath)) throw Object.assign(new Error("Document interne introuvable."), { statusCode: 404 });
  return absolutePath;
}
function internalKitCatalog() {
  const groups = new Map();
  for (const [title, group, description, relativePath] of internalKitEntries()) {
    let size = 0;
    try { size = fs.statSync(resolveInternalKitPath(relativePath)).size; } catch {}
    if (!groups.has(group)) groups.set(group, []);
    groups.get(group).push({ title, description, path: relativePath, filename: path.basename(relativePath), size });
  }
  return [...groups.entries()].map(([label, files]) => ({ label, files }));
}
function kitMimeType(filePath) {
  if (filePath.toLowerCase().endsWith(".zip")) return "application/zip";
...
async function downloadInternalKitFile(req, res) {
  const url = queryUrl(req);
  const relativePath = clean(url.searchParams.get("path") || "", 500);
  const filePath = resolveInternalKitPath(relativePath);
  res.statusCode = 200;
  res.setHeader("Content-Type", kitMimeType(filePath));
  res.setHeader("Content-Disposition", `attachment; filename="${safeFilename(path.basename(filePath))}"`);
...
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (entity === "kit") return sendJson(res, 200, { ok: true, kit: internalKitCatalog() });
      if (entity === "kit_file") return await downloadInternalKitFile(req, res);
    ...
    if (req.method === "POST") {
      if (type === "document") return sendJson(res, 201, { ok: true, document: await createDocument(data, req) });
      if (type === "template") return sendJson(res, 201, { ok: true, template: await createTemplate(data, req) });
      if (type === "generated") return sendJson(res, 201, { ok: true, ...(await generateDocument(data, req)) });
    }
    if (req.method === "PATCH") {
      if (type === "document") return sendJson(res, 200, { ok: true, document: await updateDocument(data, req) });
      if (type === "template") return sendJson(res, 200, { ok: true, template: await updateTemplate(data, req) });
    }
    sendJson(res, 405, { ok: false, error: "Methode non autorisee." });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    const setupError = statusCode >= 500 ? setupErrorMessage(error, "Documents", "supabase/tvf-os-documents.sql") : null;
    sendJson(res, setupError ? 503 : statusCode, { ok: false, error: setupError ? setupError.message : statusCode >= 500 ? "Documents temporairement non charges : vérifiez la configuration Supabase du module." : error.message, code: setupError?.code || error.code || undefined, details: process.env.NODE_ENV === "development" ? error.details || error.message : undefined });
  }
}
module.exports = handler;
module.exports._private = { normalizeSupabaseUrl, normalizeSupabaseRestUrl, inferDocumentType, inferTemplateType, documentTypeFromTemplateType, detectSensitive, documentAssistant, templateAssistant, documentPayload, templatePayload, decodeBase64File, safeFilename };
