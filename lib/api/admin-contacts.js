...
async function markMobileImported(row, contact, req) {
...
async function importMobileRequest(data, req) {
  ...
  const contact = await createContact(contactPayloadFromMobile(mobileRequest), req);
  const imported = await markMobileImported(mobileRequest, contact, req);
  await writeAdminAudit({ supabaseRequest, req, moduleKey: "requests", action: "import_mobile_request", objectType: "mobile_request", objectId: mobileRequest.id, summary: `Import de la demande mobile ${mobileRequest.reference || mobileRequest.id} vers ${contact?.request_number || "TVF OS"}.`, metadata: { contact_id: contact?.id || null, flow: mobileRequest.flow || null, category: mobileRequest.target_category || null } });
  return { contact, mobileRequest: enrichMobileRequest(imported) };
}
...
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      const data = await readJsonBody(req);
      if (data.type === "mobile-import") {
        const result = await importMobileRequest(data, req);
