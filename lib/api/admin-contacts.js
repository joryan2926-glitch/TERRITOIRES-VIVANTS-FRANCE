...
function inferPriority(input) {
  const text = normalizeForMatch([input.profile, input.subject, input.message, input.source_page, input.channel].join(" "));
  if (/effondrement|incendie|danger|fuite de gaz|personne en danger|securite|sinistre|risque|insalubre|habitat indigne|urgent|demain|immediat/.test(text)) return "urgente";
...
async function listContacts(req) {
  ...
  if (channel && channel !== "all" && CHANNEL_VALUES.has(channel)) params.set("channel", `eq.${channel}`);
  if (search) {
...
function createPayload(input) {
  ...
  return {
    ...
    internal_notes: clean(input.internal_notes || "", 5000) || (clean(input.source_page || "", 160).includes("courrier") ? "Creation manuelle depuis courrier papier numerise." : "Creation manuelle depuis la boite de reception TVF."),
...
async function deleteTableRows(table, filter = "id=not.is.null") {
  const { restUrl, key } = supabaseConfig();
  const response = await fetchWithTimeout(`${restUrl}/${table}?${filter}`, {
    method: "DELETE",
    headers: supabaseHeaders(key, { Prefer: "return=minimal" }),
  });
  const text = await response.text();
  if (!response.ok) {
    const error = new Error(`Remise a zero impossible sur ${table}.`);
    error.statusCode = 502;
    error.details = text;
    throw error;
  }
...
async function operationalReset(req, reason = "Remise a zero operationnelle TVF OS") {
  const tables = [
  ...
  await writeAdminAudit({
    ...
    objectType: "tvf_os_reception",
    objectId: null,
    summary: reason,
    metadata: { tables: done, official_email: "contact@territoiresvivantsfrance.fr" },
  });
  return { reset: true, tables: done, official_email: "contact@territoiresvivantsfrance.fr" };
}
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      const data = await readJsonBody(req);
      if (data.type === "operational-reset") {
        const result = await operationalReset(req, clean(data.reason || "", 300) || "Remise a zero operationnelle TVF OS");
