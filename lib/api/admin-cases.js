...
async function getCaseBySourceRequestId(sourceRequestId) {
  const source = clean(sourceRequestId || "", 80);
  if (!source) return null;
  const params = new URLSearchParams({ source_request_id: `eq.${source}`, select: caseSelect(), order: "created_at.desc", limit: "1" });
  const rows = await supabaseRequest(`cases?${params.toString()}`);
  return rows?.[0] ? enrichCase(rows[0]) : null;
}
async function tryAttachManualClient(created, input, req) {
  if (!created?.id) return;
  const displayName = clean(input.client_name || "", 180);
  const email = clean(input.client_email || "", 220).toLowerCase();
  const phone = clean(input.client_phone || "", 80);
  const address = clean(input.client_address || "", 260);
  if (!displayName && !email && !phone && !address) return;
  const clientType = clean(input.client_type || "autre", 80);
  const contactTypeMap = {
    proprietaire: "proprietaire",
    collectivite: "technicien",
  ...
  const contactPayload = {
    display_name: displayName || email || phone || "Interlocuteur TVF",
    email: email || null,
    phone: phone || null,
    contact_type: contactTypeMap[clientType] || "autre",
    consent_status: "unknown",
    confidentiality_level: "standard",
    source: "dossier_manuel",
    tags: ["dossier", clean(input.case_type || "autre", 80)].filter(Boolean),
    notes: [
      `Cree depuis le dossier ${created.case_number || created.id}.`,
      address ? `Adresse / commune : ${address}` : "",
      input.request_theme ? `Thematique : ${clean(input.request_theme, 120)}` : "",
      input.intake_origin ? `Origine : ${clean(input.intake_origin, 120)}` : "",
    ].filter(Boolean).join("\n"),
    next_action: "Qualifier la relation et rattacher les pieces utiles",
  };
  try {
    const contacts = await supabaseRequest("crm_contacts?select=*", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify(contactPayload) });
    const contact = contacts?.[0];
    await supabaseRequest("case_participants", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({
      case_id: created.id,
      participant_type: contact?.id ? "contact" : "external",
      contact_id: contact?.id || null,
      display_name: contactPayload.display_name,
      role_label: clientType || "interlocuteur principal",
      is_primary: true,
    }) });
    await writeAdminAudit({ supabaseRequest, req, moduleKey: "cases", action: "attach_manual_client", objectType: "case", objectId: created.id, summary: `Interlocuteur principal rattache au dossier ${created.case_number || created.id}.`, metadata: { contact_id: contact?.id || null, client_type: clientType } });
  } catch (error) {
    await writeAdminAudit({ supabaseRequest, req, moduleKey: "cases", action: "attach_manual_client_failed", objectType: "case", objectId: created.id, summary: `Rattachement CRM non bloque pour ${created.case_number || created.id}.`, metadata: { error: clean(error?.message || "Erreur CRM", 500) } }).catch(() => {});
  }
}
async function listCases(req) {
  ...
  if (q) {
    const idClause = /^[0-9a-f-]{32,36}$/i.test(q) ? `,id.eq.${q}` : "";
    params.set("or", `(case_number.ilike.*${q}*,title.ilike.*${q}*,summary.ilike.*${q}*,territory.ilike.*${q}*,assigned_to.ilike.*${q}*,source_request_id.ilike.*${q}*${idClause})`);
  }
  const rows = await supabaseRequest(`cases?${params.toString()}`);
...
async function createCase(input, req) {
  ...
  const existing = await getCaseBySourceRequestId(payload.source_request_id);
  if (existing?.id && !["cloture", "archive"].includes(existing.status)) {
    await writeAdminAudit({ supabaseRequest, req, moduleKey: "cases", action: "reuse_existing_case", objectType: "case", objectId: existing.id, summary: `Dossier existant reutilise pour la demande source ${payload.source_request_id}.`, metadata: { source_request_id: payload.source_request_id, case_number: existing.case_number || null } });
    return existing;
  }
  ...
  await tryAttachManualClient(created, input, req);
...
module.exports._private = { normalizeSupabaseRestUrl, inferCaseType, poleFromCaseType, assistantForCase, enrichCase, casePayload, checklistPayload, riskPayload, decisionPayload, getCaseBySourceRequestId };
