...
async function createDecision(input) {
  ...
  if (payload.decision_status === "validee") await updateCase({ id: payload.case_id, status: "decision_validee", decision_status: "validee", decision_summary: payload.final_decision || payload.proposed_decision, status_note: "Decision validee" });
  return rows?.[0] || null;
}
