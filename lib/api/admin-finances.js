...
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      if (type === "funder") return sendJson(res, 201, { ok: true, funder: await createRecord("funders", funderPayload(data), req, type) });
      if (type === "opportunity") return sendJson(res, 201, { ok: true, opportunity: await createRecord("funding_opportunities", opportunityPayload(data), req, type) });
      if (type === "application") return sendJson(res, 201, { ok: true, application: await createRecord("funding_applications", applicationPayload(data), req, type) });
      if (type === "budget") return sendJson(res, 201, { ok: true, budget: await enrichedBudget(await createRecord("budgets", budgetPayload(data), req, type)) });
      if (type === "line") return sendJson(res, 201, { ok: true, line: await createRecord("budget_lines", linePayload(data), req, type) });
      if (type === "expense") return sendJson(res, 201, { ok: true, expense: { ...(await createRecord("expenses", expensePayload(data), req, type)), assistant: assistantForExpense(expensePayload(data)) } });
      if (type === "payment") return sendJson(res, 201, { ok: true, payment: await createRecord("payment_records", paymentPayload(data), req, type) });
      if (type === "report") return sendJson(res, 201, { ok: true, report: await enrichedReport(await createRecord("finance_reports", reportPayload(data), req, type)) });
    ...
    if (req.method === "PATCH") {
      ...
      if (type === "funder") return sendJson(res, 200, { ok: true, funder: await updateRecord("funders", id, funderPayload(data, true), req, type) });
      if (type === "opportunity") return sendJson(res, 200, { ok: true, opportunity: await updateRecord("funding_opportunities", id, opportunityPayload(data, true), req, type) });
      if (type === "application") return sendJson(res, 200, { ok: true, application: await updateRecord("funding_applications", id, applicationPayload(data, true), req, type) });
      if (type === "budget") return sendJson(res, 200, { ok: true, budget: await enrichedBudget(await updateRecord("budgets", id, budgetPayload(data, true), req, type)) });
      if (type === "line") return sendJson(res, 200, { ok: true, line: await updateRecord("budget_lines", id, linePayload(data, true), req, type) });
      if (type === "expense") return sendJson(res, 200, { ok: true, expense: { ...(await updateRecord("expenses", id, expensePayload(data, true), req, type)), assistant: assistantForExpense(data) } });
      if (type === "payment") return sendJson(res, 200, { ok: true, payment: await updateRecord("payment_records", id, paymentPayload(data, true), req, type) });
      if (type === "report") return sendJson(res, 200, { ok: true, report: await enrichedReport(await updateRecord("finance_reports", id, reportPayload(data, true), req, type)) });
    }
    sendJson(res, 405, { ok: false, error: "Methode non autorisee." });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    sendJson(res, statusCode, { ok: false, error: statusCode >= 500 ? "Finances temporairement non chargees : vérifiez la configuration Supabase du module." : error.message, code: error.code || undefined, details: process.env.NODE_ENV === "development" ? error.details || error.message : undefined });
  }
}
