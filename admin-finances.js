...
function renderControlPanel() {
  ...
  controlEl.innerHTML = `<div class="admin-panel-head"><div><p class="section-kicker">Decision financiere</p><h3>Relier besoins, budgets et preuves</h3><p>Cette synthese aide a savoir ce qui peut etre engage, finance ou presente a un financeur sans perdre les justificatifs.</p></div><strong>${escapeHtml(status)}</strong></div><div class="finances-control-grid"><article><span>Decision suivante</span><strong>${escapeHtml(nextDecision)}</strong><small>priorite immediate</small></article><article><span>Execution</span><strong>${escapeHtml(execution)}%</strong><small>budget consomme</small></article><article><span>A approuver</span><strong>${escapeHtml(toApprove)}</strong><small>depenses</small></article><article><span>Justificatifs</span><strong>${escapeHtml(missingReceipts)}</strong><small>pieces manquantes</small></article><article><span>Finance confirme</span><strong>${money(granted)}</strong><small>accorde</small></article></div><div class="finances-control-links"><a class="btn secondary" href="admin-dossiers">Dossiers</a><a class="btn secondary" href="admin-impact">Impact</a><a class="btn secondary" href="admin-documents">Justificatifs</a><a class="btn secondary" href="admin-crm">Financeurs</a></div>`;
}
...
function currentType() {
  const map = { budgets: "budget", expenses: "expense", funders: "funder", opportunities: "opportunity", applications: "application", payments: "payment", reports: "report" };
  return map[currentView] || "budget";
}
function financeReferenceFor(item) { return item.budget_key || item.opportunity_key || item.provider_payment_id || item.id || "Non renseigne"; }
function financeOwnerFor(item) { return item.owner_name || item.funder_name || item.vendor_name || item.provider || "Non attribue"; }
function financeSummaryFor(item) { return item.summary || item.notes || item.eligibility_notes || item.restrictions || item.ai_summary || "Aucune synthese renseignee pour le moment."; }
function financeFilePanel(item) {
  const amount = itemAmount(item);
  const assistant = item.assistant || {};
  const alertCount = Array.isArray(assistant.alerts) ? assistant.alerts.length : Array.isArray(assistant.missing_fields) ? assistant.missing_fields.length : Array.isArray(assistant.missing_documents) ? assistant.missing_documents.length : 0;
  return `<section class="finances-file-panel" aria-label="Fiche financiere"><div class="finances-file-head"><div><p class="section-kicker">Fiche financiere</p><h3>${escapeHtml(financeReferenceFor(item))}</h3></div><span data-alert="${alertCount ? "true" : "false"}">${alertCount ? `${alertCount} alerte(s)` : "Controle"}</span></div><div class="finances-file-grid"><article><span>Responsable</span><strong>${escapeHtml(financeOwnerFor(item))}</strong></article><article><span>Statut</span><strong>${escapeHtml(itemStatus(item))}</strong></article><article><span>Montant</span><strong>${amount !== "" ? money(amount) : "Non renseigne"}</strong></article><article><span>Type</span><strong>${escapeHtml(itemType(item))}</strong></article><article><span>Echeance</span><strong>${escapeHtml(formatDate(item.deadline_at || item.due_at || item.reporting_due_at || item.expense_date || item.paid_at))}</strong></article><article><span>Maj</span><strong>${escapeHtml(formatDate(item.updated_at || item.created_at))}</strong></article></div></section>`;
}
function financeActionsPanel(item) {
  const followupLabel = currentView === "budgets" ? "Generer reporting" : currentView === "expenses" ? "Demander justificatif" : currentView === "opportunities" ? "Creer demande" : currentView === "applications" ? "Generer reporting" : "Copier reference";
  return `<section class="finances-actions-panel" aria-label="Actions financieres"><div><p class="section-kicker">Suite logique</p><h3>Arbitrer, justifier, reporter</h3><p>Les actions rapides aident a suivre ce qui peut etre engage, depose ou transmis a un financeur.</p></div><div class="finances-actions-grid"><button class="btn primary" type="submit" form="finances-detail-form">Enregistrer</button><button class="btn secondary" type="button" data-finances-action="review">A reviser</button><button class="btn secondary" type="button" data-finances-action="validate">Valider</button><button class="btn secondary" type="button" data-finances-action="reject">Refuser / bloquer</button><button class="btn secondary" type="button" data-finances-action="archive">Archiver</button><button class="btn secondary" type="button" data-finances-action="followup">${escapeHtml(followupLabel)}</button></div><nav class="finances-linked-nav" aria-label="Modules lies"><a href="admin-dossiers">Dossiers</a><a href="admin-documents">Justificatifs</a><a href="admin-impact">Impact</a><a href="admin-crm">Financeurs</a></nav></section>`;
}
function financeOperationalPanel(item) { return `<div class="finances-detail-grid">${financeFilePanel(item)}${financeActionsPanel(item)}</div>`; }
...
function quickStatusFor(action) {
  const maps = {
    budgets: { review: "to_review", validate: "active", reject: "to_review", archive: "archive" },
    expenses: { review: "to_approve", validate: "approved", reject: "rejected", archive: "archive" },
    funders: { review: "prospect", validate: "active", reject: "paused", archive: "archive" },
    opportunities: { review: "a_qualifier", validate: "eligible", reject: "refuse", archive: "archive" },
    applications: { review: "a_deposer", validate: "deposee", reject: "refusee", archive: "archive" },
    payments: { review: "pending", validate: "succeeded", reject: "failed", archive: "cancelled" },
    reports: { review: "to_send", validate: "sent", reject: "draft", archive: "archive" },
  };
  return maps[currentView]?.[action] || "";
}
async function quickFinanceAction(action) {
  const item = selectedItem();
  if (!item) return notify("Selectionnez un element financier.", "warning");
  if (action === "followup") {
    if (["budgets", "applications"].includes(currentView)) return generateReport();
    if (currentView === "opportunities") {
      await api("/api/admin-finances", { method: "POST", body: JSON.stringify({ type: "application", opportunity_id: item.id, application_title: `Demande - ${itemTitle(item)}`, requested_amount: item.amount_max || item.amount_min || 0, status: "brouillon", notes: `Demande creee depuis l'opportunite ${financeReferenceFor(item)}.` }) });
      currentView = "applications";
      selectedId = null;
      await loadItems();
      return notify("Demande de financement creee.", "success");
    }
    if (currentView === "expenses") {
      const text = `Justificatif attendu pour ${itemTitle(item)} - ${money(item.amount || 0)}.`;
      try { await navigator.clipboard?.writeText(text); notify("Message justificatif copie.", "success"); } catch { notify(text, "info"); }
      return;
    }
    const reference = financeReferenceFor(item);
  ...
  const payload = { id: item.id, type: currentType() };
  if (currentView === "payments") payload.payment_status = status; else payload.status = status;
  await api("/api/admin-finances", { method: "PATCH", body: JSON.stringify(payload) });
  await loadItems();
  notify("Statut financier mis a jour.", "success");
}
function bindEvents() {
...
detailEl?.addEventListener("click", (event) => {
  const button = event.target.closest("[data-finances-action]");
  if (!button) return;
  event.preventDefault();
  quickFinanceAction(button.dataset.financesAction).catch((error) => notifyError(error));
});
