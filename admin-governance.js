...
function kindForView() { return view === "decisions" ? "decision" : view === "committees" ? "committee" : view === "actions" ? "action" : "delegation"; }
function summaryFor(item) { return item.summary || item.agenda_summary || item.notes || item.scope || "Aucune synthese renseignee pour le moment."; }
function ownerFor(item) { return item.owner_name || item.delegated_to || item.facilitator_name || item.decided_by || "Non attribue"; }
function governanceFilePanel(item, assistant = {}) {
  const reference = item.decision_number || item.committee_key || item.delegation_key || item.id || "Non renseigne";
  const objectType = item.related_object_type && item.related_object_type !== "none" ? item.related_object_type : "Non rattache";
  const objectId = item.related_object_id || item.decision_id || item.committee_id || "Non renseigne";
  const score = assistant.validation_score !== undefined ? `${assistant.validation_score}%` : "Non calcule";
  return `<section class="governance-file-panel" aria-label="Fiche decision"><div class="governance-file-head"><div><p class="section-kicker">Fiche de gouvernance</p><h3>${escapeHtml(reference)}</h3></div><span>${escapeHtml(label(item.status))}</span></div><div class="governance-file-grid"><article><span>Responsable</span><strong>${escapeHtml(ownerFor(item))}</strong></article><article><span>Priorite</span><strong>${escapeHtml(item.priority_level || "Non renseignee")}</strong></article><article><span>Score validation</span><strong>${escapeHtml(score)}</strong></article><article><span>Echeance</span><strong>${escapeHtml(formatDate(item.due_at || item.scheduled_at || item.ends_at))}</strong></article><article><span>Objet lie</span><strong>${escapeHtml(objectType)}</strong><small>${escapeHtml(objectId)}</small></article><article><span>Tracabilite</span><strong>${escapeHtml(formatDate(item.updated_at || item.created_at))}</strong><small>derniere mise a jour</small></article></div></section>`;
}
function governanceActionsPanel(item) {
  const type = kindForView();
  const followupLabel = view === "decisions" ? "Creer une action issue" : view === "committees" ? "Preparer un PV" : "Copier la reference";
  return `<section class="governance-actions-panel" aria-label="Actions de gouvernance"><div><p class="section-kicker">Suite logique</p><h3>Decider, tracer, suivre</h3><p>Chaque action rapide reste modifiable et journalisee dans TVF OS.</p></div><div class="governance-actions-grid"><button class="btn primary" type="submit" form="governance-detail-form">Enregistrer</button><button class="btn secondary" type="button" data-governance-action="review" data-type="${escapeHtml(type)}">Mettre en revue</button><button class="btn secondary" type="button" data-governance-action="validate" data-type="${escapeHtml(type)}">Valider / clore</button><button class="btn secondary" type="button" data-governance-action="block" data-type="${escapeHtml(type)}">Bloquer / refuser</button><button class="btn secondary" type="button" data-governance-action="archive" data-type="${escapeHtml(type)}">Archiver</button><button class="btn secondary" type="button" data-governance-action="followup">${escapeHtml(followupLabel)}</button></div><nav class="governance-linked-nav" aria-label="Modules lies"><a href="admin-dossiers">Dossiers</a><a href="admin-documents">Documents</a><a href="admin-work">Taches</a><a href="admin-risks">Risques</a><a href="admin-finances">Finances</a></nav></section>`;
}
function renderDetail() {
  ...
  detailEl.innerHTML = `<div class="admin-panel-head"><div><p class="section-kicker">${escapeHtml(typeLabels[view])}</p><h2>${escapeHtml(titleFor(item))}</h2><p>${escapeHtml(subFor(item))}</p></div><span>${escapeHtml(label(item.status))}</span></div><section class="governance-readiness"><article><span>Validation</span><strong>${escapeHtml(assistant.validation_score ?? "--")}${assistant.validation_score !== undefined ? "%" : ""}</strong></article><article><span>Votes favorables</span><strong>${escapeHtml(assistant.approvals ?? "--")}</strong></article><article><span>Actions ouvertes</span><strong>${escapeHtml(assistant.open_actions ?? (item.status && !["done", "archive"].includes(item.status) ? 1 : 0))}</strong></article><article><span>Echeance</span><strong>${escapeHtml(formatDate(item.due_at || item.scheduled_at || item.ends_at))}</strong></article></section><div class="governance-detail-grid">${governanceFilePanel(item, assistant)}${governanceActionsPanel(item)}</div><form id="governance-detail-form" class="governance-detail-form" data-governance-detail-form><input type="hidden" name="id" value="${escapeHtml(item.id)}"><input type="hidden" name="type" value="${kindForView()}"><label>Titre<input name="title" value="${escapeHtml(titleFor(item))}"></label><label>Statut<select name="status">${statusOptions(view, item.status)}</select></label><label>Responsable<input name="owner_name" value="${escapeHtml(ownerFor(item) === "Non attribue" ? "" : ownerFor(item))}"></label><label>Priorite<input name="priority_level" value="${escapeHtml(item.priority_level || "")}"></label><label class="governance-wide-field">Synthese<textarea name="summary" rows="5">${escapeHtml(summaryFor(item))}</textarea></label></form>`;
}
...
function quickStatusFor(action) {
  const maps = {
    decisions: { review: "to_validate", validate: "validated", block: "rejected", archive: "archive" },
    committees: { review: "in_progress", validate: "closed", block: "cancelled", archive: "archive" },
    actions: { review: "in_progress", validate: "done", block: "blocked", archive: "archive" },
    delegations: { review: "active", validate: "active", block: "paused", archive: "archive" },
  };
  return maps[view]?.[action] || "";
}
async function quickGovernanceAction(action) {
  const item = selectedItem();
  if (!item) return notify("Selectionnez un element de gouvernance.", "warning");
  if (action === "followup") {
    if (view === "decisions") {
      await api("/api/admin-governance", { method: "POST", body: JSON.stringify({ type: "action", decision_id: item.id, action_title: `Suivre - ${titleFor(item)}`, owner_name: ownerFor(item) === "Non attribue" ? "" : ownerFor(item), priority_level: item.priority_level || "moyen", notes: `Action issue de la decision ${item.decision_number || item.id}.` }) });
      await loadAll();
      return notify("Action issue creee.", "success");
    }
    if (view === "committees") return generateMinutes();
    const reference = item.decision_number || item.committee_key || item.delegation_key || item.id;
  ...
  await api("/api/admin-governance", { method: "PATCH", body: JSON.stringify({ id: item.id, type: kindForView(), status }) });
  await loadAll();
  notify("Statut de gouvernance mis a jour.", "success");
}
...
detailEl?.addEventListener("click", (event) => {
  ...
  quickGovernanceAction(button.dataset.governanceAction).catch((e) => notifyError(e));
});
