...
function instructionActions(item, type) {
  ...
  const requestHref = item.email ? "admin-demandes?q=" + encodeURIComponent(item.email) : "admin-demandes";
  const phone = type === "organization" ? item.phone : (item.phone || item.mobile);
  return `<section class="crm-instruction-actions"><div class="admin-panel-head"><div><p class="section-kicker">Parcours d'instruction</p><h4>Enregistrer, qualifier et rattacher</h4><p>Les actions principales sont regroupees pour eviter les doublons de boutons et guider le traitement du dossier.</p></div></div><div class="crm-action-row"><button class="btn primary" type="submit">Enregistrer la fiche</button><button class="btn secondary" type="button" data-crm-partner-action="case">Creer dossier d'instruction</button><button class="btn secondary" type="button" data-crm-partner-action="qualify">Ajouter note de qualification</button><button class="btn secondary" type="button" data-crm-partner-action="task">Creer tache de suivi</button><button class="btn secondary" type="button" data-crm-add-history>Note libre</button><a class="btn secondary" href="${escapeHtml(requestHref)}">Demandes liees</a>${email ? `<a class="btn secondary" href="mailto:${escapeHtml(email)}">Ecrire</a>` : ""}${phone ? `<a class="btn secondary" href="tel:${escapeHtml(phone)}">Appeler</a>` : ""}</div></section>`;
}
