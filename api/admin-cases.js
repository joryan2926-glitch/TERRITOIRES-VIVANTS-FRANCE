...
async function updateCase(input) {
  ...
  if (payload.status && previous?.status !== payload.status) await supabaseRequest("case_status_history", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify(historyPayload(id, previous?.status, payload.status, input.status_note)) });
...
async function createRisk(input) {
...
async function createDecision(input) {
  ...
  if (payload.decision_status === "validee") await updateCase({ id: payload.case_id, status: "decision_validee", decision_status: "validee", decision_summary: payload.final_decision || payload.proposed_decision });
  return rows?.[0] || null;
}
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      if (type === "case") return sendJson(res, 201, { ok: true, case: await createCase(data) });
      if (type === "risk") return sendJson(res, 201, { ok: true, risk: await createRisk(data) });
      if (type === "decision") return sendJson(res, 201, { ok: true, decision: await createDecision(data) });
    }
    if (req.method === "PATCH") {
      if (type === "case") return sendJson(res, 200, { ok: true, case: await updateCase(data) });
      if (type === "checklist") return sendJson(res, 200, { ok: true, item: await updateChecklist(data) });
    }
    sendJson(res, 405, { ok: false, error: "Methode non autorisee." });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    sendJson(res, statusCode, { ok: false, error: statusCode >= 500 ? "Dossiers indisponibles pour le moment." : error.message, code: error.code || undefined, details: process.env.NODE_ENV === "development" ? error.details || error.message : undefined });
  }
}
