...
async function generateReport(input) {
  ...
  const report = await createRecord("impact_reports", reportPayload({ report_title: title, report_type: input.report_type || "national", status: publishable.length ? "to_validate" : "draft", summary: `Bilan genere depuis ${publishable.length} indicateur(s) publiable(s) et ${proofs.length} preuve(s) validee(s).`, metric_ids: publishable.map((m) => m.id), proof_ids: proofs.map((p) => p.id).slice(0, 60), ai_summary: `Bilan impact pret pour revue : ${publishable.length} indicateur(s) prouve(s).` }));
...
async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === "POST") {
      if (type === "metric") return sendJson(res, 201, { ok: true, metric: await enrichedMetric(await createRecord("impact_metrics", metricPayload(data))) });
      if (type === "value") return sendJson(res, 201, { ok: true, value: await createRecord("impact_values", valuePayload(data)) });
      if (type === "proof") return sendJson(res, 201, { ok: true, proof: await createRecord("impact_proofs", proofPayload(data)) });
      if (type === "report") return sendJson(res, 201, { ok: true, report: await createRecord("impact_reports", reportPayload(data)) });
      if (type === "export") return sendJson(res, 201, { ok: true, export_job: await createRecord("impact_exports", exportPayload(data)) });
    ...
    if (req.method === "PATCH") {
      ...
      if (type === "metric") return sendJson(res, 200, { ok: true, metric: await enrichedMetric(await updateRecord("impact_metrics", id, metricPayload(data, true))) });
      if (type === "value") return sendJson(res, 200, { ok: true, value: await updateRecord("impact_values", id, valuePayload(data, true)) });
      if (type === "proof") return sendJson(res, 200, { ok: true, proof: await updateRecord("impact_proofs", id, proofPayload(data, true)) });
      if (type === "report") return sendJson(res, 200, { ok: true, report: await updateRecord("impact_reports", id, reportPayload(data, true)) });
      if (type === "export") return sendJson(res, 200, { ok: true, export_job: await updateRecord("impact_exports", id, exportPayload(data, true)) });
    }
    sendJson(res, 405, { ok: false, error: "Methode non autorisee." });
  } catch (error) {
    const statusCode = error.statusCode || 500;
    sendJson(res, statusCode, { ok: false, error: statusCode >= 500 ? "Impact indisponible pour le moment." : error.message, code: error.code || undefined, details: process.env.NODE_ENV === "development" ? error.details || error.message : undefined });
  }
}
