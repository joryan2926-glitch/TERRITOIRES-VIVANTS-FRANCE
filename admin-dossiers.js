...
function clientDossierSchemaPanel(item) {
  const source = item.source_request_id ? `Demande rattachee : ${item.source_request_id}` : "Demande source a rattacher";
  ...
  const steps = [
    ...
    ["Categorie", category],
    ["Statut", status],
    ["N° dossier", item.case_number || "Generation automatique"],
    ["Fichier client", source],
    ["Documents", "Pieces, courriers, conventions et preuves"],
  ...
  return `<section class="cases-panel cases-client-schema-panel" aria-label="Schema de construction du dossier client">
    <div class="admin-panel-head"><div><p class="section-kicker">Fichier client</p><h4>Construction du dossier par etape</h4><p>Chaque dossier doit permettre de retrouver la demande, les coordonnees, la categorie, le statut, les documents et le module d'instruction.</p></div><strong>${escapeHtml(item.case_number || "Dossier")}</strong></div>
