...
function casePriorityReason(item) {
  ...
  if (item.risk_level === "critique" || item.risk_level === "eleve") return "Risque a cadrer";
...
function moduleChainPanel(item) {
  ...
  return [
    ...
    { key: "decision", title: "Note de decision", type: "compte_rendu", summary: `Note preparatoire pour decision humaine : contexte, risques, pieces, recommandation et prochaine etape.` }
  ];
}
function documentsPanel(item) {
  const docs = documentPackForCase(item);
  return `<section class="cases-panel cases-documents-panel"><div class="admin-panel-head"><div><p class="section-kicker">Documents utiles</p><h4>Preparer les pieces du dossier</h4></div><strong>${docs.length}</strong></div><p>Ces actions creent des brouillons rattaches au dossier dans le module Documents. Ils restent a verifier avant envoi, signature ou usage officiel.</p><div class="cases-documents-grid">${docs.map((doc) => `<article><strong>${escapeHtml(doc.title)}</strong><span>${escapeHtml(label(typeLabels, item.case_type))}</span><p>${escapeHtml(doc.summary)}</p><button class="btn secondary" type="button" data-case-document="${escapeHtml(doc.key)}">Preparer</button></article>`).join("")}</div><div class="admin-detail-actions"><button class="btn primary" type="button" data-case-document="pack">Creer le pack documentaire</button><a class="btn secondary" href="admin-documents">Ouvrir Documents</a></div></section>`;
}
...
function instructionControlPanel(item) {
  ...
  const cards = [
    ...
    { label: "Risque", value: label(riskLabels, riskLevel), detail: `${risks.length} risque(s) trace(s)`, tone: riskLevel === "critique" || riskLevel === "eleve" ? "danger" : "neutral" },
  ...
  return `<section class="cases-control-panel" aria-label="Pilotage du dossier">
    <div class="admin-panel-head">
      <div>
        ...
        <p>Ce resume aide a decider quoi faire avant toute convention, visite, communication ou engagement externe.</p>
      </div>
    ...
    <div class="cases-control-actions">
      ...
      <button class="btn secondary" type="button" data-case-document="pack">Pack documentaire</button>
