...
const statusLabels = { ouvert: "Nouveau dossier", qualification: "Qualification en cours", instruction: "Instruction", attente_pieces: "Documents attendus", contact: "Contact en cours", attente_proprietaire: "En attente proprietaire", visite: "Visite a programmer", visite_realisee: "Visite realisee", etude: "Etude en cours", solutions: "Recherche de solutions", partenaires: "Partenaires mobilises", financement: "Financement recherche", plan_action: "Plan d action propose", convention: "Convention en preparation", projet_engage: "Projet engage", intervention: "Intervention en cours", controle_final: "Controle final", remis_usage: "Bien remis en usage", a_decision: "Decision a preparer", decision_validee: "Decision validee", suspendu: "Dossier suspendu", abandonne: "Dossier abandonne", cloture: "Dossier cloture", archive: "Archive" };
...
function propertyAndOwnerPanel(item) {
  const participant = primaryParticipant(item);
  const propertyAddress = safeCaseValue(item.property_address, item.address, item.raw_address, item.territory);
  const propertyType = label(typeLabels, item.case_type);
  const owner = safeCaseValue(participant?.display_name, item.client_name, item.owner_name, "Proprietaire a identifier");
  const ownerRole = safeCaseValue(participant?.role_label, item.client_type, item.source_request_id ? "Demande source" : "Contact a rattacher");
  const cadastral = safeCaseValue(item.cadastral_reference, item.parcel_reference, item.parcelle, "Reference non saisie");
  return `<section class="cases-panel cases-operational-panel cases-property-panel"><div class="admin-panel-head"><div><p class="section-kicker">Bien immobilier</p><h4>Fiche du bien et proprietaire rattache</h4><p>Un bien peut recevoir plusieurs demandes ou dossiers dans le temps.</p></div><a class="text-link" href="admin-map?q=${encodeURIComponent(propertyAddress)}">Carte</a></div><div class="cases-phase-grid"><article><span>Adresse ou secteur</span><strong>${escapeHtml(propertyAddress)}</strong><small>${escapeHtml(item.territory || "Territoire a confirmer")}</small></article><article><span>Type de bien</span><strong>${escapeHtml(propertyType)}</strong><small>${escapeHtml(item.main_pole || "Pole a definir")}</small></article><article><span>Proprietaire / contact</span><strong>${escapeHtml(owner)}</strong><small>${escapeHtml(ownerRole)}</small></article><article><span>Cadastre / doublon</span><strong>${escapeHtml(cadastral)}</strong><small>Verification adresse, GPS, parcelle et contact</small></article></div><div class="cases-control-actions"><a class="btn secondary" href="admin-crm?view=proprietaires&q=${encodeURIComponent(owner)}">Fiche proprietaire</a><a class="btn secondary" href="admin-map?q=${encodeURIComponent(propertyAddress)}">Localiser</a><button class="btn secondary" type="button" data-case-create-task>Verifier doublon</button></div></section>`;
}
function guidedInstructionPanel(item) {
  const workflow = workflowForCase(item);
  const phases = [["Qualification", "Adresse, categorie, doublons, priorite"], ["Recherche fonciere", "Cadastre, proprietaire, droits connus"], ["Contact", "Courrier, appel, e-mail, relance"], ["Visite", "Photos, grille terrain, compte rendu"], ["Analyse", "Technique, juridique, financier, risques"], ["Solution", "Partenaires, aides, convention, calendrier"], ["Mise en oeuvre", "Interventions, materiaux, preuves"], ["Cloture", "Remise en usage, bilan, impact"]];
  return `<section class="cases-panel cases-guided-panel"><div class="admin-panel-head"><div><p class="section-kicker">Instruction guidee</p><h4>Prochaine etape recommandee</h4><p>${escapeHtml(workflow.objective)}</p></div><strong>${escapeHtml(label(statusLabels, item.status))}</strong></div><ol class="cases-phase-path">${phases.map((phase, index) => `<li class="${index <= Math.min(caseInstructionIndex(item.status), phases.length - 1) ? "is-done" : ""}"><span>${index + 1}</span><strong>${escapeHtml(phase[0])}</strong><small>${escapeHtml(phase[1])}</small></li>`).join("")}</ol><div class="cases-control-actions"><button class="btn secondary" type="button" data-case-workflow="pieces">Documents manquants</button><button class="btn secondary" type="button" data-case-workflow="rdv">Programmer visite</button><button class="btn secondary" type="button" data-case-workflow="convention">Preparer convention</button></div></section>`;
}
function ecosystemPanel(item) {
  const partners = [["Collectivite", item.territory || "Territoire a cadrer"], ["Entreprise / artisan", item.contractor_name || "Prestataire a identifier"], ["Financeur", item.funder_name || "Aide a rechercher"], ["Association / usage", item.project_user || "Usage a construire"]];
  const materials = item.case_type === "materiaux" ? "Ressource principale du dossier" : "A verifier selon travaux";
  return `<section class="cases-panel cases-ecosystem-panel"><div class="admin-panel-head"><div><p class="section-kicker">Ecosysteme operationnel</p><h4>Partenaires, financements, materiaux et interventions</h4><p>Phase 5 : mobiliser les acteurs utiles sans melanger contact, dossier et intervention.</p></div><a class="text-link" href="admin-crm?view=organizations">Annuaire</a></div><div class="cases-phase-grid">${partners.map(([name, detail]) => `<article><span>${escapeHtml(name)}</span><strong>${escapeHtml(detail)}</strong><small>Fiche, convention, role et historique</small></article>`).join("")}<article><span>Programmes et aides</span><strong>${escapeHtml(item.funding_status || "Correspondance a etudier")}</strong><small>Criteres, pieces et echeances</small></article><article><span>Banque de materiaux</span><strong>${escapeHtml(materials)}</strong><small>Entrees, sorties, reservations, affectation</small></article></div><div class="cases-control-actions"><a class="btn secondary" href="admin-crm?view=organizations">Partenaires</a><a class="btn secondary" href="admin-impact?view=programmes">Programmes et aides</a><a class="btn secondary" href="admin-documents?view=materiaux">Banque de materiaux</a></div></section>`;
}
function visitAndInterventionPanel(item) {
  const address = safeCaseValue(item.property_address, item.address, item.raw_address, item.territory);
  const visitStatus = item.status === "visite_realisee" ? "Visite realisee" : item.status === "visite" ? "Visite a programmer" : "Visite selon besoin";
  const interventionStatus = ["intervention", "controle_final", "remis_usage"].includes(item.status) ? label(statusLabels, item.status) : "Non engagee";
  return `<section class="cases-panel cases-visit-panel"><div class="admin-panel-head"><div><p class="section-kicker">Terrain et interventions</p><h4>Visites, travaux, photos et suivi</h4><p>Les donnees terrain doivent pouvoir remonter depuis TVF Mobile et rester rattachees au dossier.</p></div><a class="text-link" href="admin-work?view=interventions">Planning</a></div><div class="cases-phase-grid"><article><span>Adresse d'intervention</span><strong>${escapeHtml(address)}</strong><small>Itineraire et verification terrain</small></article><article><span>Visite</span><strong>${escapeHtml(visitStatus)}</strong><small>Photos, grille, compte rendu</small></article><article><span>Intervention</span><strong>${escapeHtml(interventionStatus)}</strong><small>Devis, facture, materiaux, avancement</small></article><article><span>Cloture terrain</span><strong>${escapeHtml(item.status === "remis_usage" ? "Remise en usage" : "A documenter")}</strong><small>Avant / apres, controle final, impact</small></article></div><div class="cases-control-actions"><button class="btn secondary" type="button" data-case-quick="visite">Programmer visite</button><a class="btn secondary" href="admin-work?view=interventions">Creer intervention</a><a class="btn secondary" href="admin-documents">Photos et rapports</a></div></section>`;
}
