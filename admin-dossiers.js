...
async function loadCases() { if (countEl) countEl.textContent = "Chargement des dossiers..."; const result = await api(`/api/admin-cases?${filtersParams().toString()}`); cases = result.cases || []; if (!cases.some((item) => item.id === selectedId)) selectedId = cases[0]?.id || null; await loadDashboard().catch(() => {}); renderTabs(); renderList(); renderDetail(); }
...
async function addRisk() { const item = selectedCase(); if (!item) return; const risk_label = prompt("Risque identifie"); if (!risk_label) return; const mitigation = prompt("Mesure de maitrise") || "A definir"; await api("/api/admin-cases", { method: "POST", body: JSON.stringify({ type: "risk", case_id: item.id, risk_label, mitigation }) }); await loadCases(); }
async function addDecision() { const item = selectedCase(); if (!item) return; const proposed_decision = prompt("Decision proposee"); if (!proposed_decision) return; await api("/api/admin-cases", { method: "POST", body: JSON.stringify({ type: "decision", case_id: item.id, proposed_decision, decision_status: "proposee" }) }); await loadCases(); }
function openModal() { if (modal) modal.hidden = false; modalForm?.querySelector("input, select, textarea")?.focus(); }
function closeModal() { if (modal) modal.hidden = true; }
async function createCase(event) { event.preventDefault(); const statusEl = modalForm.querySelector("[data-cases-modal-status]"); const data = Object.fromEntries(new FormData(modalForm)); data.type = "case"; try { statusEl.hidden = false; statusEl.textContent = "Creation..."; const result = await api("/api/admin-cases", { method: "POST", body: JSON.stringify(data) }); cases = [result.case, ...cases]; selectedId = result.case.id; modalForm.reset(); closeModal(); renderList(); renderDetail(); await loadDashboard().catch(() => {}); } catch (error) { statusEl.hidden = false; statusEl.textContent = error.message; } }
function exportCsv() { if (!cases.length) return alert("Aucun dossier a exporter."); const headers = ["Numero", "Titre", "Type", "Statut", "Priorite", "Pole", "Responsable", "Maturite", "Prochaine action"]; const rows = cases.map((item) => [item.case_number, item.title, label(typeLabels, item.case_type), label(statusLabels, item.status), label(priorityLabels, item.priority), item.main_pole, item.assigned_to, item.maturity_score, item.next_action]); const csv = [headers, ...rows].map((row) => row.map((v) => `"${String(v || "").replace(/"/g, '""')}"`).join(";")).join("\n"); const blob = new Blob(["\ufeff" + csv], { type: "text/csv;charset=utf-8" }); const url = URL.createObjectURL(blob); const link = document.createElement("a"); link.href = url; link.download = `dossiers-tvf-${new Date().toISOString().slice(0, 10)}.csv`; document.body.appendChild(link); link.click(); link.remove(); URL.revokeObjectURL(url); }
function bindEvents() { tokenForm?.addEventListener("submit", async (event) => { event.preventDefault(); const value = String(new FormData(tokenForm).get("token") || "").trim(); if (!value) return setStatus("Entrez le token admin.", "error"); setToken(value); try { showApp(); await loadCases(); setStatus(""); } catch (error) { setToken(""); showLogin(); setStatus(error.message, "error"); } }); filtersForm?.addEventListener("input", () => { clearTimeout(debounceTimer); debounceTimer = setTimeout(() => loadCases().catch((e) => alert(e.message)), 280); }); filtersForm?.addEventListener("change", () => loadCases().catch((e) => alert(e.message))); statusButtons.forEach((button) => button.addEventListener("click", () => { if (statusFilter) statusFilter.value = button.dataset.caseStatus || "all"; loadCases().catch((e) => alert(e.message)); })); refreshButton?.addEventListener("click", () => loadCases().catch((e) => alert(e.message))); logoutButton?.addEventListener("click", () => { setToken(""); window.location.href = "admin"; }); exportButton?.addEventListener("click", exportCsv); createButton?.addEventListener("click", openModal); closeModalButtons.forEach((button) => button.addEventListener("click", closeModal)); modal?.addEventListener("click", (event) => { if (event.target === modal) closeModal(); }); modalForm?.addEventListener("submit", createCase); listEl?.addEventListener("click", (event) => { const button = event.target.closest("[data-case-id]"); if (!button) return; selectedId = button.dataset.caseId; renderList(); renderDetail(); }); detailEl?.addEventListener("submit", (event) => { const form = event.target.closest("[data-cases-detail-form]"); if (!form) return; event.preventDefault(); saveDetail(form); }); detailEl?.addEventListener("change", (event) => { const select = event.target.closest("[data-checklist-id]"); if (select) updateChecklist(select.dataset.checklistId, select.value).catch((e) => alert(e.message)); }); detailEl?.addEventListener("click", (event) => {
    ...
    if (docButton) createCaseDocument(docButton.dataset.caseDocument).catch((e) => alert(e.message));
