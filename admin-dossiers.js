...
function historyPanel(items) { return `<section class="cases-panel"><p class="section-kicker">Timeline</p>${items.length ? items.map((h) => `<article><strong>${escapeHtml(label(statusLabels, h.to_status))}</strong><span>${escapeHtml(formatDate(h.created_at))} - ${escapeHtml(h.note || "Changement de statut")}</span></article>`).join("") : "<p>Aucun historique.</p>"}</section>`; }
