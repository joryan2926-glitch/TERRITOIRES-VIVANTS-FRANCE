...
function showApp() { if (loginSection) loginSection.hidden = true; if (appSection) appSection.hidden = false; document.body?.classList.add("admin-session-active"); }
function showLogin() { if (loginSection) loginSection.hidden = false; if (appSection) appSection.hidden = true; document.body?.classList.remove("admin-session-active"); }
...
async function loadInternalKit() {
  ...
  if (!internalKit.length) {
    internalKit = internalKitFallback;
    renderInternalKit();
  }
  try {
    const result = await api("/api/admin/documents?entity=kit");
    if (Array.isArray(result.kit) && result.kit.length) {
...
async function hasServerSession() {
  try {
    ...
    return response.ok && result?.session?.authenticated === true;
  } catch { return false; }
}
async function initDocumentsModule() {
  if (token()) {
    showApp();
    await loadInitialDocumentsApp();
    return;
  }
  ...
  showLogin();
  if (!loginSection) window.location.href = "admin-login";
}
