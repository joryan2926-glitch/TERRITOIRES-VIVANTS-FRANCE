...
function renderGlobalControlPanel() {
  ...
  const nextDecision = toClassify ? "Classer les documents" : toValidate ? "Valider les pieces" : sensitive ? "Controler les acces" : "Maintenir la bibliotheque";
  globalControlEl.innerHTML = `<div class="admin-panel-head"><div><p class="section-kicker">Pilotage documentaire</p><h3>Garder des pieces propres, tracables et utilisables</h3><p>Cette synthese distingue les documents a classer, les pieces a valider, les elements sensibles et les modeles officiels.</p></div><strong>${escapeHtml(nextDecision)}</strong></div><div class="documents-control-grid"><article data-tone="warning"><span>A classer</span><strong>${escapeHtml(toClassify)}</strong><small>tri requis</small></article><article data-tone="success"><span>A valider</span><strong>${escapeHtml(toValidate)}</strong><small>decision humaine</small></article><article data-tone="danger"><span>Sensibles</span><strong>${escapeHtml(sensitive)}</strong><small>acces prudent</small></article><article><span>Modeles officiels</span><strong>${escapeHtml(official)}/${escapeHtml(totalTemplates)}</strong><small>national</small></article><article><span>Action</span><strong>${escapeHtml(nextDecision)}</strong><small>priorite</small></article></div><div class="documents-control-actions"><a class="btn secondary" href="admin-dossiers">Dossiers</a><a class="btn secondary" href="admin-procedures">Procedures</a><a class="btn secondary" href="admin-governance">Validation</a><a class="btn secondary" href="admin-risks">Confidentialite</a></div>`;
...
async function generateFromTemplate(id) {
  ...
  const title = prompt("Titre du brouillon genere", template?.title ? `Brouillon - ${template.title}` : "Brouillon TVF");
  if (!title) return;
  const relatedType = prompt("Rattachement du brouillon : case, request, contact, organization, project, branch ou none", "case") || "none";
  const relatedId = relatedType !== "none" ? (prompt("ID de l'objet rattache, si connu") || "") : "";
  const fieldValues = {};
  (template?.required_fields || []).slice(0, 12).forEach((field) => {
    fieldValues[field] = prompt(`Champ a renseigner - ${field}`, "") || "";
  });
  const result = await api("/api/admin-documents", { method: "POST", body: JSON.stringify({ type: "generated", template_id: id, title, generated_from_object_type: relatedType, generated_from_object_id: relatedId, field_values: fieldValues }) });
  view = "documents";
...
async function copyRelatedId(id) {
  if (!id) return;
  try { await navigator.clipboard.writeText(id); } catch { prompt("ID rattache", id); }
}
