...
def add_signature_block(doc, left_label="Pour TVF", right_label="Pour le demandeur / partenaire"):
...
FORMS = [
    ...
    },
    {
        ...
        "title": "Demande collectivite - territoire partenaire",
    ...
    },
    {
        ...
        "title": "Demande entreprise - partenariat territorial",
...
CONVENTIONS = [
    ...
    },
    {
        "ref": "TVF-CONV-04",
        "folder": "13-conventions-types",
        "title": "Convention type de partenariat entreprise",
...
def add_common_form_sections(doc: Document, form: dict):
    ...
    paragraph(doc, "Identification du dossier", style="Heading 2")
    add_kv_table(
        doc,
        [
            ("Numero dossier TVF", "TVF-____-____"),
            ("Date de reception", "____ / ____ / ______"),
            ("Canal d'entree", "[ ] Site web   [ ] E-mail   [ ] Telephone   [ ] Rendez-vous   [ ] Autre"),
            ...
            ("Statut", "[ ] Recu   [ ] Pieces demandees   [ ] En instruction   [ ] Rendez-vous   [ ] Classe"),
        ],
        header="Zone reservee a TVF",
    )

    paragraph(doc, "Informations a completer par le demandeur", style="Heading 2")
    rows = [(field, "________________________________________________________________________") for field in form["fields"]]
    add_kv_table(doc, rows, widths=(2.25, 4.0))

    paragraph(doc, "Description libre", style="Heading 2")
    for _ in range(8):
        paragraph(doc, "_" * 92, after=2)

    paragraph(doc, "Pieces a fournir ou a verifier", style="Heading 2")
    add_check_table(doc, "Checklist de recevabilite du dossier", form["pieces"])

    paragraph(doc, "Instruction TVF", style="Heading 2")
    add_numbered(
        doc,
        [
            "Verifier la coherence de la demande et l'identite du demandeur.",
            "Controler les pieces transmises et demander les elements manquants.",
            "Evaluer l'interet territorial, la faisabilite, les risques et les partenaires a mobiliser.",
            "Orienter vers le bon parcours : rendez-vous, visite, convention, classement ou reponse ecrite.",
            "Enregistrer la decision et la prochaine action dans TVF OS.",
        ],
    )
    add_kv_table(
        doc,
        [
            ("Suite proposee", form["next"]),
            ("Pieces manquantes", "____________________________________________________________"),
            ("Decision TVF", "[ ] Poursuivre   [ ] Rendez-vous   [ ] Visite   [ ] Convention   [ ] Classer"),
            ("Prochaine action", "____________________________________________________________"),
        ],
        widths=(1.8, 4.45),
    )

    paragraph(doc, "Clauses et engagements adaptes a cette demande", style="Heading 2")
...
def build_convention(conv: dict) -> Path:
    ...
    paragraph(doc, "Parties", style="Heading 1")
    add_kv_table(
        doc,
        [
            ...
            ("Partenaire", "Nom / structure : _________________________________________________"),
            ("Representant partenaire", "Nom : ____________________________   Fonction : ____________________________"),
            ("Adresse partenaire", "____________________________________________________________"),
        ],
        widths=(1.75, 4.5),
    )
    paragraph(doc, "Objet", style="Heading 1")
    paragraph(doc, conv["object"])
    paragraph(doc, "Cadre operationnel", style="Heading 2")
    add_kv_table(
        doc,
        [
            ("Lieu concerne", "____________________________________________________________"),
            ("Duree", "Du ____ / ____ / ______ au ____ / ____ / ______"),
            ("Objectifs", "____________________________________________________________"),
            ("Referent TVF", "____________________________________________________________"),
            ("Referent partenaire", "____________________________________________________________"),
            ("Frequence de suivi", "[ ] Hebdomadaire   [ ] Mensuelle   [ ] A chaque etape   [ ] Autre"),
        ],
        widths=(1.75, 4.5),
    )
    paragraph(doc, "Clauses types", style="Heading 1")
    add_bullets(doc, conv["clauses"])
    paragraph(doc, "Points a verrouiller avant toute action", style="Heading 2")
    add_check_table(
        doc,
        "Controle prealable obligatoire",
        [
            ("Pouvoir de signature / autorisation", "Obligatoire", "Verifier representant habilite."),
            ("Assurance", "Obligatoire", "Identifier qui couvre quoi."),
            ("Securite / acces", "Obligatoire", "Aucune action sans conditions de securite."),
            ("Budget / couts", "A verifier", "Identifier depenses et prise en charge."),
            ("Communication", "A verifier", "Valider les mentions publiques."),
            ("Donnees personnelles", "A verifier", "Limiter les donnees au strict necessaire."),
        ],
    )
    paragraph(doc, "Annexes recommandees", style="Heading 2")
    add_bullets(doc, conv["annexes"])
    paragraph(doc, "Suivi et bilan", style="Heading 2")
    add_kv_table(
        doc,
        [
...
def build_piece_list() -> Path:
    doc = setup_doc(
        ...
        "Demandeurs, partenaires et equipe TVF",
    )
...
def build_letters() -> Path:
    doc = setup_doc(
        "Kit de courriers prets a remplir",
        "TVF-COUR-01",
    ...
    out = OUT / "15-courriers-prets-a-envoyer" / "tvf-cour-01-kit-courriers-demandes.docx"
    save_doc(doc, out)
    return out
...
def build_index(paths: list[Path]) -> Path:
    doc = setup_doc(
        "Index du kit operationnel TVF",
        "TVF-INDEX-01",
        "Repertoire des formulaires, conventions, listes de pieces et courriers prets a utiliser.",
        "Equipe TVF et partenaires",
        status="Repertoire de classement",
    )
    paragraph(doc, "Objectif du kit", style="Heading 1")
    paragraph(
        doc,
        "Ce kit regroupe les modeles documentaires utiles pour receptionner une demande, qualifier un besoin, demander les pieces, instruire le dossier, preparer une convention et transmettre un courrier adapte.",
    )
    ...
    paragraph(doc, "Documents generes dans ce kit", style="Heading 2")
    for path in sorted(paths):
        rel = path.relative_to(OUT)
        p = doc.add_paragraph(style="List Bullet")
        add_run(p, str(rel), color=DARK, size=9)
    out = OUT / "00-index" / "tvf-index-kit-formulaires-conventions.docx"
    save_doc(doc, out)
    return out
...
def build_readme(paths: list[Path]) -> None:
    lines = [
        "# Kit formulaires et conventions TVF",
        "",
        "Ce dossier regroupe les modeles pret a remplir pour la gestion operationnelle de Territoires Vivants France.",
        "",
        "## Organisation",
        "",
        "- `00-index` : repertoire general du kit.",
