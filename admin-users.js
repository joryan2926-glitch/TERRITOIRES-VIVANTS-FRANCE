...
function membershipsList(profile) { const rows = profile.memberships || []; if (!rows.length) return `<p class="form-note">Aucun rattachement antenne.</p>`; return `<div class="users-row-list">${rows.map((row) => `<article><div><strong>${escapeHtml(row.primary_role_label || row.branch_id || "Antenne")}</strong><span>${escapeHtml(label(row.membership_status))}</span></div><small>${escapeHtml(formatDate(row.joined_at || row.updated_at))}</small></article>`).join("")}</div>`; }
function renderDetail() {
  ...
  const profileBlocks = view === "profiles" ? `<section class="users-readiness"><article><span>Score</span><strong>${escapeHtml(assistant.access_score ?? "--")}%</strong></article><article><span>Roles actifs</span><strong>${escapeHtml(assistant.active_roles ?? 0)}</strong></article><article><span>Antennes</span><strong>${escapeHtml(assistant.active_memberships ?? 0)}</strong></article><article><span>Revues</span><strong>${escapeHtml(assistant.pending_reviews ?? 0)}</strong></article></section><h3>Roles attribues</h3>${rolesList(item)}<h3>Rattachements antennes</h3>${membershipsList(item)}` : "";
